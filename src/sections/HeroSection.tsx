      ...
      <div className="mx-auto grid w-full max-w-6xl items-center gap-14 px-5 py-16 lg:grid-cols-[1.1fr_.9fr]">
        <motion.div {...reveal}>
          <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-violet-400/20 bg-violet-400/10 px-4 py-2 text-sm font-semibold text-violet-300 light:text-violet-700">
            <Sparkles size={15} aria-hidden="true" /> Disponível para novos desafios
          </span>
          ...
          <h2 className="mt-6 text-xl font-bold text-slate-200 sm:text-2xl light:text-slate-700">{siteConfig.role}</h2>
          <p className="mt-5 max-w-xl text-base leading-8 text-slate-400 sm:text-lg light:text-slate-600">
            Construo soluções robustas combinando{' '}
            <strong className="text-slate-200 light:text-slate-800">Java, Spring Boot, React, Cloud e Inteligência Artificial</strong>.
          </p>
          ...
          <div className="mt-9 flex gap-3" aria-label="Redes e contato">
            <a className="social-button" aria-label="GitHub" href={siteConfig.github} target="_blank" rel="noopener noreferrer">
              <FaGithub size={19} aria-hidden="true" />
            </a>
            <a className="social-button" aria-label="LinkedIn" href={siteConfig.linkedin} target="_blank" rel="noopener noreferrer">
              <FaLinkedin size={19} aria-hidden="true" />
            </a>
            <a className="social-button" aria-label="E-mail" href={`mailto:${siteConfig.email}`}>
        ...
        <motion.div {...revealWithDelay(0.1)} className="relative mx-auto w-full max-w-md">
          ...
          <div className="absolute -bottom-5 -left-5 rounded-2xl border border-white/10 bg-slate-900/85 px-5 py-4 backdrop-blur-xl light:bg-white/90">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Stack</p>
            <p className="mt-1 font-bold">Java • React • IA</p>
          </div>
        </motion.div>
      </div>

      <a href="#about" aria-label="Ir para a seção sobre" className="absolute bottom-7 left-1/2 -translate-x-1/2 animate-bounce text-slate-500">
